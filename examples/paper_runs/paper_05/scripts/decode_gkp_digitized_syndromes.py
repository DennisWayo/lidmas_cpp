#!/usr/bin/env python3
"""Decode paper_05 digitized-GKP syndrome request streams."""
...
from paper05_decoder_policies import decode_policy, parse_decoders, supports_to_check_matrix, syndrome_from_request
...
def request_paths(in_dir: Path) -> list[Path]:
    manifest_path = in_dir / "ingest_manifest.csv"
    if manifest_path.exists():
        with manifest_path.open("r", encoding="utf-8", newline="") as f:
            rows = list(csv.DictReader(f))
        paths = [in_dir / row["request_file"] for row in rows if row.get("request_file")]
        return sorted(paths)
    return sorted(in_dir.glob("decoder_requests_*.ndjson"))
...
def main() -> int:
    ...
    for req_path in request_paths(in_dir):
        dataset = req_path.stem.replace("decoder_requests_", "", 1)
        resp_paths = {
            decoder: out_dir / f"decoder_responses_{dataset}_gkp_surface_{decoder}.ndjson"
        ...
        with contextlib.ExitStack() as stack:
            ...
            for line in req_f:
                ...
                distance = int(meta.get("distance", "5"))
                if distance not in matrices:
                    geom = build_surface_geometry(distance)
                    matrices[distance] = supports_to_check_matrix(geom.n_data, geom.z_supports)
                matrix = matrices[distance]
                syndrome = syndrome_from_request(rec)
                injected_raw = meta.get("injected_q", "")
                ...
                for decoder in decoders:
                    ...
                    if injected != "":
                        exact_match: int | str = int(correction == [injected])
                        contains_target: int | str = int(injected in correction)
                    else:
                    ...
                    diagnostics = {
                        **result.diagnostics,
                        ...
                        "n_qubits": meta.get("n_data", rec.get("n_qubits", "")),
                    ...
                    response = {
                        "correction": {
                            ...
                            "decoder_name": f"gkp_surface_{decoder}",
                        },
    ...
    print(f"Decoded {len(csv_rows)} digitized-GKP syndrome-policy rows into {out_dir}")
    return 0
