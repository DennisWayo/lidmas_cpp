#!/usr/bin/env python3
"""Analyze paper_05 digitized-GKP syndrome correction results."""
...
def dataset_label(dataset: str) -> str:
    return short_dataset_label(dataset)


def circuit_label(circuit_id: str) -> str:
    if circuit_id == "clean":
        return "clean"
    if circuit_id.startswith("q_shift_data_"):
        return f"q{circuit_id.removeprefix('q_shift_data_')}"
...
def circuit_sort_key(circuit_id: str) -> tuple[int, int | str]:
    ...
    if circuit_id.startswith("q_shift_data_"):
        try:
            return (1, int(circuit_id.removeprefix("q_shift_data_")))
        except ValueError:
            pass
    return (2, circuit_id)
...
def summarize(rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for (dataset, circuit_id), group in sorted(groups.items(), key=lambda item: (item[0][0], circuit_sort_key(item[0][1]))):
        injected_values = {r.get("injected_q", "") for r in group if r.get("injected_q", "") != ""}
...
def plot_syndrome_heatmap(rows: list[dict[str, str]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    ordered = sorted(groups, key=lambda key: (key[0], circuit_sort_key(key[1])))
    labels = [compact_source_label(dataset, circuit) for dataset, circuit in ordered]
    ...
    ax.set_xlabel("stream (PL=PennyLane)")
    ax.set_ylabel("outer Z-check bit")
    ...
    save_fig(fig, out_dir / "figure_gkp_digitized_syndrome_heatmap", manuscript_dir)
    plt.close(fig)
...
def plot_correction_match(summary_rows: list[dict[str, Any]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    inj_rows = [r for r in summary_rows if str(r.get("injected_q", "")) != ""]
    if not inj_rows:
        return
    targets = sorted({int(str(r["injected_q"])) for r in inj_rows})
    row_by_target = {int(str(r["injected_q"])): r for r in inj_rows}
    x = np.arange(len(targets))

    fig, ax = plt.subplots(figsize=half_panel_size("rate"), constrained_layout=True)
    for field, low_field, high_field, label, color, marker in [
        ("exact_intended_match_rate", "exact_intended_match_ci95_low", "exact_intended_match_ci95_high", "exact", "#2563EB", "o"),
        (
        ...
        values = np.asarray([f(row_by_target[target][field]) for target in targets], dtype=float)
        lows = np.asarray([f(row_by_target[target][low_field]) for target in targets], dtype=float)
        highs = np.asarray([f(row_by_target[target][high_field]) for target in targets], dtype=float)
        ax.errorbar(
            x,
            values,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"q{target}" for target in targets])
    style_rate_axis(ax, ymin=0.25, ymax=0.75)
    ax.legend(frameon=False, loc="upper right", handlelength=1.4)
    save_fig(fig, out_dir / "figure_gkp_digitized_correction_localization", manuscript_dir)
    plt.close(fig)
...
def plot_correction_volume(summary_rows: list[dict[str, Any]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    row_by_circuit = {str(r["circuit_id"]): r for r in summary_rows}
    x = np.arange(len(circuit_ids))
    values = np.asarray([f(row_by_circuit[circuit]["mean_correction_weight"]) for circuit in circuit_ids], dtype=float)

    fig, ax = plt.subplots(figsize=half_panel_size("bar"), constrained_layout=True)
    ax.bar(x, values, width=0.62, color=GKP_COLOR)
    ax.set_ylabel("mean correction weight")
    ax.set_xticks(x)
    ax.set_xticklabels([circuit_label(circuit) for circuit in circuit_ids])
    style_bar_axis(ax)
    save_fig(fig, out_dir / "figure_gkp_digitized_correction_volume", manuscript_dir)
    plt.close(fig)
...
def main() -> int:
    ...
    write_csv(out_dir / "table_gkp_digitized_syndrome_summary.csv", summary_rows, fields)
    ...
    print(f"Wrote paper_05 digitized-GKP analysis to {out_dir}")
    return 0
