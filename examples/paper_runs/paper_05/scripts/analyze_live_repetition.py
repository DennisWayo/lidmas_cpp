#!/usr/bin/env python3
"""Analyze paper_05 live repetition-code syndrome correction results."""
...
def plot_syndrome_heatmap(rows: list[dict[str, str]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    ax.set_yticklabels([f"S{i}" for i in range(max_checks)])
    ax.set_xlabel("stream (I=IBM, L=local)")
    ax.set_ylabel("syndrome check")
    ...
    save_fig(fig, out_dir / "figure_live_syndrome_heatmap", manuscript_dir)
    plt.close(fig)
...
def plot_correction_match(summary_rows: list[dict[str, Any]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    save_fig(fig, out_dir / "figure_correction_localization", manuscript_dir)
    plt.close(fig)
...
def plot_correction_volume(summary_rows: list[dict[str, Any]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    style_bar_axis(ax)
    ax.legend(frameon=False, loc="upper left", ncol=len(dataset_keys))
    save_fig(fig, out_dir / "figure_correction_volume", manuscript_dir)
    plt.close(fig)
...
def main() -> int:
    ...
    write_csv(
        out_dir / "table_live_syndrome_summary.csv",
        summary_rows,
        [
            "dataset",
            "source",
    ...
    import matplotlib.pyplot as plt  # noqa: F401  # type: ignore
    ...
    print(f"Wrote paper_05 analysis to {out_dir}")
    return 0
