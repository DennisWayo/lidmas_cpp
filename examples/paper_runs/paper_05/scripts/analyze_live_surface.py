#!/usr/bin/env python3
"""Analyze paper_05 live surface-code syndrome correction results."""
...
def plot_syndrome_heatmap(rows: list[dict[str, str]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    save_fig(fig, out_dir / "figure_surface_syndrome_heatmap", manuscript_dir)
    plt.close(fig)
...
def plot_correction_volume(summary_rows: list[dict[str, Any]], out_dir: Path, manuscript_dir: Path | None) -> None:
    ...
    save_fig(fig, out_dir / "figure_surface_correction_volume", manuscript_dir)
    plt.close(fig)
...
def main() -> int:
    ...
    write_csv(out_dir / "table_surface_syndrome_summary.csv", summary_rows, fields)
    ...
    print(f"Wrote paper_05 surface-code analysis to {out_dir}")
    return 0
