#!/usr/bin/env python3
"""Convert raw paper_05 surface-code syndrome results into decoder requests."""
...
from surface_syndrome import build_surface_geometry, expected_syndrome, parse_bitstring, sanitize_label, syndrome_to_events
...
def main() -> int:
        ...
        with req_path.open("w", encoding="utf-8") as req_f, truth_path.open("w", encoding="utf-8") as truth_f:
            for experiment in payload.get("experiments", []):
                ...
                for shot_index, shot in enumerate(_iter_shots(experiment)):
                    ...
                    syndrome, data_bits = parse_bitstring(bitstring, geom)
                    rec = {
                        "code_id": f"surface_d{distance}_z_checks",
                        ...
                        "metadata": {
                            ...
                            "generator": "paper05_surface_z_syndrome",
                            "code_family": "surface",
                        ...
                        json.dumps(
                            {
                                ...
                                "logical_observable": "surface_z_check_single_x",
                                "logical_truth": 0,
                            },
                            separators=(",", ":"),
                        )
    ...
    print(f"Wrote {len(manifest_rows)} surface-code request streams to {out_dir}")
    return 0
